...
describe("Join Us Tests", () => {
  ...
  context("Mobile Viewport (400, 800)", () => {
    ...
    it('Checks join us flow on Mobile', function() {
      ...
      cy.get('.group > .relative').click();
      cy.get('.pt-8 > :nth-child(2)').should('have.text', ' Enter your email ');
      cy.get('.mb-8 > :nth-child(1) > .text-white').should('have.text', 'Continue with Google');
      cy.get('.mb-8 > :nth-child(2) > .text-white').should('have.text', 'Continue with Facebook');
      cy.get('.mb-8 > :nth-child(3) > .text-white').should('have.text', 'Continue with Apple');
      cy.get('.px-4 > .group > .h-full').clear('mi');
      cy.get('.px-4 > .group > .h-full').type('michael');
      cy.get('.px-4 > .group > .absolute').should('have.text', 'Invalid email address');
      ...
      /* ==== End Cypress Studio ==== */
      /* ==== Generated with Cypress Studio ==== */
      cy.get('.gap-6 > :nth-child(2) > .text-xs').should('have.text', ' Upload a profile picture  and complete your  presentation ');
      cy.get('.gap-6 > :nth-child(2) > .mb-1').should('have.text', ' Nice to meet you, ');
      /* ==== End Cypress Studio ==== */
    });
  });
});
